"""MongoDB connection helpers for ai-service-v2."""
...
class MongoManager:
    ...
    async def connect(self) -> None:
        ...
        logger.info("MongoDB conectado para ai-service-v2")

    async def close(self) -> None:
        """Close the MongoDB client."""
        if self.client is not None:
            self.client.close()
            logger.info("MongoDB fechado para ai-service-v2")

    def get_collection(self, collection_name: str):
        """Return a collection handle from the active database."""
        if self.database is None:
            raise RuntimeError("MongoDB nao foi inicializado no ai-service-v2")
