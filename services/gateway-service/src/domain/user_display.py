...
def first_name_from_text(value: Optional[str]) -> Optional[str]:
    """Return a safe first name, avoiding raw emails and technical ids."""
    if not value:
        return None
...
def first_name_from_user(user: Optional[Dict[str, Any]], fallback: Optional[str] = None) -> Optional[str]:
