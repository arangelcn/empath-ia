"""Conversation identity helpers copied into the unified boundary."""
...
def split_composite_session_id(session_id: str) -> tuple[Optional[str], str]:
    """Split ``username_session-N`` while allowing underscores in usernames."""
    if not session_id:
...
def build_legacy_session_id(
    ...
    """Build the legacy Mongo key used by existing conversation documents."""
