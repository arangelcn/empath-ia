...
async def get_realtime_emotion_stats(
    ...
    """Return realtime emotion stats compatible with the legacy admin contract."""
    user_emotions = container.mongo.get_collection("user_emotions")
    last_24h = datetime.now(UTC) - timedelta(hours=24)

    total_detections = await user_emotions.count_documents({"timestamp": {"$gte": last_24h}})
    emotion_stats = []
    async for result in user_emotions.aggregate(
        [
        ...
        emotion_stats.append(
            {
                "emotion": result.get("_id") or "neutro",
                "count": count,
                "avg_confidence": round(float(result.get("avg_confidence") or 0), 2),
                "percentage": round((count / total_detections) * 100, 2) if total_detections else 0,
            }
        )

    unique_users = len(
        await user_emotions.distinct(
            "username",
            {"timestamp": {"$gte": last_24h}},
        )
    )
    face_detections = await user_emotions.count_documents(
        {
            "timestamp": {"$gte": last_24h},
            "face_detected": True,
        }
    )
    face_detection_rate = round((face_detections / total_detections) * 100, 2) if total_detections else 0
